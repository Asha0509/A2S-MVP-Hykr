            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div className="mb-8">
                    ...
                    <h1 className="font-serif text-4xl sm:text-5xl text-main leading-tight font-black italic">
                        Empty room in. <span className="text-accent">Magazine cover out.</span>
                    </h1>
                ...
                <div className="grid lg:grid-cols-5 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-5">
                        ...
                        >
                            ...
                            {previewUrl ? (
                                <div className="space-y-3">
                                    ...
                                    <p className="text-xs text-muted text-center">Click or drop another file to replace</p>
                                </div>
                            ) : (
                                <div className="text-center py-6">
                                    ...
                                    <p className="text-xs text-muted mt-1">or click to choose · JPG / PNG / WebP · up to 12 MB</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="rounded-2xl bg-surface border border-premium p-5 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-main mb-2">Style</label>
                                <div className="grid grid-cols-2 gap-2">
                                    {STYLES.map((s) => (
                                        ...
                                        >
                                            <p className="text-sm font-semibold text-main">{s.label}</p>
                                            <p className="text-[11px] text-muted leading-snug mt-0.5">{s.blurb}</p>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full rounded-lg border border-premium bg-main px-3 py-2 text-sm text-main focus:outline-none focus:ring-2 focus:ring-accent"
                                />
                                <p className="text-[11px] text-muted text-right mt-1">{hint.length}/400</p>
                            </div>
                            ...
                            <button
                                ...
                                style={{ backgroundColor: (loading || !file) ? '#9CA3AF' : 'var(--accent)', color: '#ffffff' }}
                                className="w-full inline-flex items-center justify-center gap-2 rounded-lg font-semibold py-3 text-sm hover:opacity-90 disabled:cursor-not-allowed transition"
                            >
                                {loading ? (
                                    <>
                    ...
                    <div className="lg:col-span-3">
                        <div className="rounded-2xl bg-surface border border-premium p-5 min-h-[420px] flex flex-col">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="font-serif text-xl text-main font-black italic">Staged result</h2>
                                {result && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <a
                                            ...
                                            className="inline-flex items-center gap-1.5 rounded-lg text-xs font-semibold px-3 py-1.5 hover:opacity-90"
                                        >
                            ...
                            {!result && !loading && (
                                <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-10 border-2 border-dashed border-premium rounded-xl">
                                    <div className="w-14 h-14 rounded-full bg-accent/10 flex items-center justify-center mb-4">
                                        <ImagePlus size={26} className="text-accent" />
                                    </div>
                                    <p className="text-muted max-w-sm">
                                        Your AI-staged room will appear here. Pick a photo, choose a style, then hit&nbsp;
                                        <span className="font-semibold text-accent">Stage this room</span>.
                                    </p>
                                </div>
                            )}
                            ...
                            {loading && (
                                <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-10">
                                    <RefreshCw size={28} className="text-accent animate-spin mb-4" />
                                    <p className="text-main font-semibold">
                                        Restaging in {STYLES.find(s => s.value === style)?.label || style} style…
                                    </p>
                                    <p className="text-xs text-muted mt-2">This usually takes 8–20 seconds. Don't refresh.</p>
                                </div>
                            )}
                                ...
                                <div className="space-y-4">
                                    <div className="grid sm:grid-cols-2 gap-3">
                                        <figure className="space-y-1">
                                            ...
                                            <figcaption className="text-[11px] text-muted uppercase tracking-wide font-semibold text-center">Before</figcaption>
                                        </figure>
                                        <figure className="space-y-1">
                                            <img src={stagedDataUrl} alt="Staged" className="w-full rounded-xl object-cover aspect-[4/3] ring-2 ring-accent/40" />
                                            <figcaption className="text-[11px] text-accent uppercase tracking-wide font-semibold text-center">
                                                After · {result.style}
                                            </figcaption>
                                        </figure>
                                    </div>
                                    ...
                                    {result.caption && (
                                        <p className="text-sm text-muted italic border-l-2 border-accent pl-3">
                                            {result.caption}
                                        </p>
                                    )}
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-2 pt-2">
                                        <Link
                                            to="/gallery"
                                            style={{ backgroundColor: 'var(--accent)', color: '#ffffff' }}
                                            className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg font-semibold py-2.5 text-sm hover:opacity-90"
                                        >
                                        ...
                                        <Link
                                            to="/vastu-score"
                                            className="flex-1 inline-flex items-center justify-center gap-2 rounded-lg border border-accent text-accent font-semibold py-2.5 text-sm hover:bg-accent/5"
                                        >
                                            Check Vastu score
                                        </Link>
                                    </div>
